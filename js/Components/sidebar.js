...
import { compterConversationsNonLues, compterSignalementsATraiter } from "../Utils/badgeHelpers.js";
...
export function renderSidebar() {
  ...
  const items = filteredLinks.map(link => `
    ...
    >
      ...
      ${["messagerie", "signalements"].includes(link.page) ? `
        <span id="navBadge-${link.page}" class="nav-badge ml-auto hidden h-5 min-w-[1.25rem] flex-shrink-0 items-center justify-center rounded-full bg-bloque px-1.5 text-[10px] font-black text-white"></span>
      ` : ""}
    </button>
  `).join("");
...
export function initSidebar() {
  ...
  setInterval(refreshSidebarBadges, 20000);

  return { close: closeSidebar };
}

export async function refreshSidebarBadges() {
  const [nbMessages, nbSignalements] = await Promise.all([
    compterConversationsNonLues(),
    compterSignalementsATraiter(),
  ]);

  updateBadge("messagerie", nbMessages);
  updateBadge("signalements", nbSignalements);
}
